...
const newSpace = (blogData = {}) => {
    const content = blogData.fields.name || '';
    const photo = blogData.uploadPath;
    const createtime = Date.now()
    const sql = `insert into space (content,photo,createtime)
		values ('${content}','${photo}',${createtime})`
    return exec(sql).then(inserData =>{
        return {id: inserData.insertId}
    })
}

const uploadPhoto= function (req,dirName,callback){
    //上传文件
    // parse a file upload
